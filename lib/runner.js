...
function initMocha(baseOptions) {
    ...
    function shimMochaInstance(m) {
        ...
        m.setup = (options) => {
            ...
            return setup(Object.assign({ reporter: 'spec' }, baseOptions, options));
        };
...
function createIncognitoContextIfRequested(incognito, browser) {
    return incognito ? browser.createIncognitoBrowserContext() : Promise.resolve(browser);
}
...
exports.runner = function ({ file, grep, invert, reporter, timeout, width, height, args, executablePath, incognito, visible }) {
    return new Promise(resolve => {
        ...
        const mochaOptions = { grep, invert, reporter };

        const result = puppeteer
            .launch(options)
            .then(browser => createIncognitoContextIfRequested(incognito, browser)
                .then(browserOrContext => browserOrContext.newPage())
                ...
                .then(page => {
                    ...
                    return page.evaluateOnNewDocument(initMocha, mochaOptions)
                        .then(() => page.goto(url))
